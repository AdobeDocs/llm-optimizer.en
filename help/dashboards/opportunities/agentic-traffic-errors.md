...
It surfaces two key metrics for each error type:
...
LLM Optimizer queries your CDN logs via Athena to identify URLs returning 404, 403, or 5xx status codes to AI agent user agents — including ChatGPT, Claude, Perplexity, and Gemini. Up to 500 URLs per audit are analyzed, ranked by traffic volume. The audit runs weekly by default.
...
For **404 errors**, suggestions are AI-powered — LLM Optimizer analyzes the broken URL's structure and content to recommend alternative URLs that preserve user intent, along with a confidence score and rationale explaining the recommendation.

For **403 and 5xx errors**, suggestions are template-based, providing targeted guidance for each error type.

## Error details table

The error details table lists all affected URLs, filterable by **User Agent**, **Country Code**, and **Category**. For each URL it shows:
...
- A **Stats** chart showing total hits per week by AI agent user agent, so you can understand the traffic impact and whether the issue is persistent or improving
...
See the Agentic Traffic Errors opportunity in action using the Frescopa demo environment.
